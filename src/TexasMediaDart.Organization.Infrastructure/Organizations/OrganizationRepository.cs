...
public sealed class OrganizationRepository : IOrganizationRepository
{
    ...
    public async Task<CurrentOrganizationDto> UpdateAsync(
        Guid identityUserId,
        string name,
        bool isActive,
        string modifiedBy,
        CancellationToken cancellationToken = default)
    {
        ...
        var command = new CommandDefinition(
            commandText: "[dbo].[sp_Organization_Update]",
            parameters: new
            {
                IdentityUserId = identityUserId,
                Name = name,
        ...
        return await connection.QuerySingleAsync<CurrentOrganizationDto>(
            command);
    }

    public async Task<IReadOnlyList<UserModulePermissionDto>>
        GetUserModulesAsync(
            Guid identityUserId,
            CancellationToken cancellationToken = default)
    {
        ...
        var modules =
            await connection.QueryAsync<UserModulePermissionDto>(
                command);
